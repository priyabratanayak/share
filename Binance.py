...
class Binance:
    ...
    def GetSymbolKlines(self, symbol:str, interval:str, limit:int=1000, end_time=False):
        ...
        df['open_date_time']=[np.datetime64(x).astype("float") for x in df.date]
        df['open_date_time']=df['open_date_time']/1000
        df['Dates2'] = pd.to_datetime(df['open_date_time'] * 1000000, infer_datetime_format=True)
