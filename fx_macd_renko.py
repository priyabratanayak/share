...
def main():
    try:
        #for every single currency, whether we have any existing open position.
            ...
            if len(open_pos)>0:
                ...
                if len(open_pos_cur)>0:
                    if open_pos_cur["isBuy"].tolist()[0]==True:
                        ...
                        long_short = "short"   
            data = con.get_candles(currency, period='m5', number=250)#250 rows of 5 min candle
...
while time.time() <= timeout:
    try:
        ...
        main()
        time.sleep(300 - ((time.time() - starttime) % 300.0)) # 5 minute interval between each new execution. As we are working with 5 min candle
