...
for ticker in tickers:
    renko_param[ticker] = {"brick_size":round(atr(fetchOHLC(ticker,"5minute",15),200),2),"upper_limit":None, "lower_limit":None,"brick":0}#"brick":0 number of continuous bricks forms. to start with number of bricks=0
...
def on_connect(ws,response):
    ws.subscribe(tokens)
    ws.set_mode(ws.MODE_LTP,tokens)#we are using LTP mode instead of full mode
