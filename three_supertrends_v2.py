...
def st_dir_refresh(ohlc,ticker):
    ...
    global st_dir
    #to check for the reversal. That means the close price changes from below the supertrend line
    #to above the super trend line
    ...
    if ohlc["st3"][-1] > ohlc["close"][-1] and ohlc["st3"][-2] < ohlc["close"][-2]:
        st_dir[ticker][2] = "red"
    #to check for the reversal. That means the close price changes from above the supertrend line
    #to below the super trend line
    if ohlc["st1"][-1] < ohlc["close"][-1] and ohlc["st1"][-2] > ohlc["close"][-2]:
...
def sl_price(ohlc):
    ...
    st = ohlc.iloc[-1,[-3,-2,-1]]#Taking last 3 columns of hlc df.
    #last three columns contains super trend line 1,2, and 3
    if st.min() > ohlc["close"][-1]:#It happend during the doen trend.
    #Super trend line will be above the candle
...
def placeSLOrder(symbol,buy_sell,quantity,sl_price):    
    ...
    #Placing the combination of 2 orders one is market rder and the second is the stop loss order
    # We can do the same with bracket order. But the bracket order is somewhat behaves in unpredicatble manner.
    #so we have custmised our own order
...
#For existing orders it updates the stop loss level
...
def main(capital):
    a,b = 0,0
    #based on the candle size whether it is 5 mins or 1 hr you need to call both position df and order df
    #ord_df is used to update the stop loss level if needed
    
    #postion data frame pos_df tells you all the positions you have taken throughout the day. We will get all open positions from here
 
    #Order data frame ord_df tells you all the orders you have placed throughout the day. We will get all order pending or pending stocks
    #As there tend to be failure rate of kite.position api call we need to try it multiple times
    ...
    for ticker in tickers:
        ...
        try:
            ...
            ohlc["st1"] = supertrend(ohlc,7,3)#super trend 1
            ohlc["st2"] = supertrend(ohlc,10,3)#super trend 2
            ohlc["st3"] = supertrend(ohlc,11,2)#super trend 3
            st_dir_refresh(ohlc,ticker)
            quantity = int(capital/ohlc["close"][-1])#Quantity we want to buy. We can also use kite.ltp() that gives us the last traded price fro the stock
            #when you start the day pos_df is blank
            ...
            if len(pos_df.columns)!=0 and ticker in pos_df["tradingsymbol"].tolist():
                if pos_df[pos_df["tradingsymbol"]==ticker]["quantity"].values[0] == 0:
                    ...
                    if st_dir[ticker] == ["red","red","red"]:
                        placeSLOrder(ticker,"sell",quantity,sl_price(ohlc))
                if pos_df[pos_df["tradingsymbol"]==ticker]["quantity"].values[0] != 0:
                    #As we are using same sl trigger price and the sl price we wil lvalidate only "TRIGGER PENDING" as shown below.
                    #Otherwise we need to validate "Pending" and "TRIGGER PENDING"
                    order_id = ord_df.loc[(ord_df['tradingsymbol'] == ticker) & (ord_df['status'].isin(["TRIGGER PENDING","OPEN"]))]["order_id"].values[0]
                    #Update the stop loss based on the new stop loss price
...
while time.time() <= timeout:
    try:
        main(capital)
        time.sleep(300 - ((time.time() - starttime) % 300.0))#sleeps for 5 mins
