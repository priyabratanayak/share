...
def app():
    ...
    if 'placeholder_instrumentval' not in st.session_state:
        st.session_state['placeholder_instrumentval']=" "
    ...
    with st.form(key="buy"):
            ...
            with col2:
                st.session_state.placeholder_instrument=st.empty()
                st.session_state.instrument = st.session_state.placeholder_instrument.text_input("Instrument / Order ID")
            ...
            with col5:
                ...
                buybtn = st.form_submit_button(label="Buy")
                if buybtn:
                ...
                if cancel:
                    ...
                    # in the dataframe refer order_id
                    if len(st.session_state.instrument)>0:
                        symbol=st.session_state.instrument
                        try:
            ...
            if net_df.shape[0]>0:
                ...
                net_df=net_df[net_df['transaction_type']=='BUY']
